...
def copy_models(output_dir: Path, selected_models: list[str]) -> None:
    for item in get_model_copy_plan(selected_models):
        ensure_exists(item.source, f"Model asset '{item.source}'")
...
set "PATH=%ROOT%{DEFAULT_RUNTIME_DIRNAME};%ROOT%{DEFAULT_RUNTIME_DIRNAME}\\Scripts;%ROOT%{DEFAULT_RUNTIME_DIRNAME}\\Library\\bin;%ROOT%{DEFAULT_RUNTIME_DIRNAME}\\DLLs;%ROOT%{DEFAULT_RUNTIME_DIRNAME}\\Lib\\site-packages\\PySide6;%ROOT%inference\\qwen3asr_dml\\bin;%PATH%"
set "QT_PLUGIN_PATH=%ROOT%{DEFAULT_RUNTIME_DIRNAME}\\Lib\\site-packages\\PySide6\\plugins"
set "QT_QPA_PLATFORM_PLUGIN_PATH=%ROOT%{DEFAULT_RUNTIME_DIRNAME}\\Lib\\site-packages\\PySide6\\plugins\\platforms"
cd /d "%ROOT%"
"""
